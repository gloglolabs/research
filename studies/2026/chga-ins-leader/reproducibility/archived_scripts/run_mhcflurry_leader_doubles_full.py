...
OUT=HERE/'mhcflurry_leader_doubles_full';OUT.mkdir(exist_ok=True)
base=json.loads((HERE/'scan_manifest.json').read_text())
leader_manifest=json.loads((HERE.parent/'leader_design/double_scan/scan_manifest.json').read_text())
...
metadata=dict(code_version=mhcflurry.__version__,torch_version=torch.__version__,python=platform.python_version(),
    ...
    scope='Canonical13double-variant/control full104residue sensitivity; no experimental construct claimed; endogenousCHGAunchanged')
