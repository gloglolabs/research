...
OUT=HERE/'mhcflurry_all_variants';OUT.mkdir(exist_ok=True)
...
original=base['constructs'][1]
constructs=[dict(id=f'CANONICAL_CHGA18_T17{x}_INS25_SENSITIVITY',sequence=original['sequence'][:16]+x+original['sequence'][17:]) for x in 'ACDEFGHIKLMNPQRSTVWY']
assert original['sequence'][16]=='T'
...
metadata=dict(code_version=mhcflurry.__version__,torch_version=torch.__version__,python=platform.python_version(),
    ...
    scope='Canonical all20 T17-residue computational sensitivity only; no functional result or actual experimental construct claimed')
